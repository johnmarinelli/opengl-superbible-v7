...
class IndexedDraw : public john::Application
{
public:
  IndexedDraw();
...
void IndexedDraw::render(double current_time) {
...
void IndexedDraw::on_key(int key, int action)
{
}

void IndexedDraw::on_resize(int w, int h)
{
  width = w;
  height = h;
  aspect = (float) w / (float) h;
  proj_matrix = glm::perspective(50.0f, aspect, 0.1f, 1000.0f);
  printf("IndexedDraw::on_resize(%d, %d)\n", width, height);
}

void IndexedDraw::on_mouse(int button, int action) 
{
}

void IndexedDraw::handle_click() 
{
}

void IndexedDraw::shutdown()
{
...
{
  chapter7::IndexedDraw* app = new chapter7::IndexedDraw;
  app->run(app);
